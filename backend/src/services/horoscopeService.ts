type StarSign = 'Aries' | 'Taurus' | 'Gemini' | 'Cancer' | 'Leo' | 'Virgo' |
    'Libra' | 'Scorpio' | 'Sagittarius' | 'Capricorn' | 'Aquarius' | 'Pisces' | 'NotSure';

interface HoroscopeReadings {
    overall: string[];
    love: string[];
    career: string[];
    wellness: string[];
...
const HOROSCOPE_READINGS: Record<StarSign, HoroscopeReadings> = {
    Aries: {
        overall: [
...
export const generateDailyHoroscope = (sign: StarSign, date: string) => {
    ...
    const overallIndex = Math.floor(seededRandom(seed + signSeed) * readings.overall.length);
    const loveIndex = Math.floor(seededRandom(seed + signSeed + 1) * readings.love.length);
    const careerIndex = Math.floor(seededRandom(seed + signSeed + 2) * readings.career.length);
    const wellnessIndex = Math.floor(seededRandom(seed + signSeed + 3) * readings.wellness.length);
    const moodIndex = Math.floor(seededRandom(seed + signSeed + 4) * readings.moods.length);
    const colorIndex = Math.floor(seededRandom(seed + signSeed + 5) * LUCKY_COLORS.length);
    const compatIndex = Math.floor(seededRandom(seed + signSeed + 6) * COMPATIBILITY_SIGNS.length);
    const luckyNum = Math.floor(seededRandom(seed + signSeed + 7) * 99) + 1;
...
export const generateDailyGuidance = (sign: StarSign, date: string) => {
    const dateParts = date.split('-').map(Number);
    const seed = dateParts[0] * 10000 + dateParts[1] * 100 + dateParts[2];
